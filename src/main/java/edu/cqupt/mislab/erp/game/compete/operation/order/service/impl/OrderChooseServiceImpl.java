...
public class OrderChooseServiceImpl implements OrderChooseService {
    ...
    public void test3(){
        ...
        //查看当前比赛进行到那一轮了
        final GameOrderChooseInfo gameOrderChooseInfo = gameOrderChooseInfoRepository.findByOrderModelInfoBasicInfo_GameBasicInfo_Id(gameId);

        //获取当前执行到哪一个步骤了
        final OrderModelInfoBasicInfo orderModelInfoBasicInfo = gameOrderChooseInfo.getOrderModelInfoBasicInfo();

        //选取该步骤的全部订单的顺序
        final Long infoId = orderModelInfoBasicInfo.getId();

        final List<EnterpriseAdInfo> enterpriseAdInfos = enterpriseAdInfoRepository.findByOrderModelInfoBasicInfo_IdAndFinishedIsFalseOrderByMoneyDescIdAsc(infoId);

        //获取当前的订单的顺序
        final int concurrentIndex = enterpriseAdInfos.indexOf(gameOrderChooseInfo.getConcurrentEnterprise());

        if(concurrentIndex == -1){

            gameOrderChooseInfo.setConcurrentEnterprise(enterpriseAdInfos.get(0));
        }
